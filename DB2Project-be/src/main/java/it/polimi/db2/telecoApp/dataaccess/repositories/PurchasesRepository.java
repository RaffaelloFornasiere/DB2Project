...
public interface PurchasesRepository extends JpaRepository<OrderEntity, Long> {
    ...
    Optional<OrderEntity> findPurchaseEntityByPurchaseDateAndUserEntity(LocalDateTime purchase_date, UserEntity user);

    //sometimes tho, the methods are so long that you feel bad just watching them. Let's give an example:
    Optional<OrderEntity> findDistinctFirstByPurchaseDateAfterAndUserEntityBirthdateAndPackageEntityNameOrPurchaseDateLessThanOrderById(LocalDateTime purchaseDate, LocalDate userEntity_birthdate, String packageEntity_name, LocalDateTime purchaseDate2);
    ...
    List<OrderEntity> findAllByUserEntity_NameAndUserEntity_Surname(String userEntity_name, String userEntity_surname);


    //here is the same method but with a jpql query
    @Query(
            "select p from OrderEntity p join p.userEntity u where u.name = :name and u.surname = :surname"
    )
    List<OrderEntity> findAllIdByUserNameAndSurname(String name, String surname);
    ...
    List<OrderEntity> findAllIdByUserNameAndSurnameNATIVE(String name, String surname);
